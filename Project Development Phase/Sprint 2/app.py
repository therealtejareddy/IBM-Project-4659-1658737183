...
@app.route('/')
def home():
    message = "TEAM ID : PNT2022TMID03404" +" "+ "BATCH ID : B1-1M3E "
...
def custdelet(ID):
    ...
    if student:
        try:
            sql = f"delete from ISSUE where id='{escape(ID)}' AND STATUS = 'Completed'"
            stmt = ibm_db.exec_immediate(conn, sql)
            return redirect(url_for("welcome"))
        except:
            return redirect(url_for("welcome"))
...
def agentdelete(ID):
    sql = f"select * from AGENT where Id='{escape(ID)}'"
    print(sql)
    stmt = ibm_db.exec_immediate(conn, sql)
    student = ibm_db.fetch_row(stmt)
    if student:
        sql = f"delete from AGENT where id='{escape(ID)}'"
        stmt = ibm_db.exec_immediate(conn, sql)
        users = []
        flash("Delected Successfully", "success")
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if account:
                ...
                print("exec")
                insert_sql = "INSERT INTO AGENT (NAME,EMAIL,PASSWORD,PHONENUMBER,SERVICE_AGENT,ADDRESS,CITY,STATE,COUNTRY,RESUME_LINK) VALUES(?,?,?,?,?,?,?,?,?,?)"
                prep_stmt = ibm_db.prepare(conn, insert_sql)
